...
@KeepForDJVM
@CordaSerializable
data class Balance<T : Any>(val quantity: Long, val displayTokenSize: BigDecimal, val token: T) : Comparable<Balance<T>> {
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun <T : Any> fromAmount(amount: Amount<T>): Balance<T> {
            return Balance(amount.quantity, amount.displayTokenSize, amount.token)
        }
    ...
    private fun checkToken(other: Balance<T>) {
    ...
    fun splitEvenly(partitions: Int): List<Balance<T>> {
        ...
        val splitAmount = Balance(commonTokensPerPartition, displayTokenSize, token)
        val splitAmountPlusOne = Balance(commonTokensPerPartition + 1L, displayTokenSize, token)
 ...
 */
data class SourceAndAmount<T : Any, out P : Any>(val source: P, val amount: Balance<T>, val ref: Any? = null)
...
class AmountTransfer<T : Any, P : Any>(val quantityDelta: Long,
    ...
    companion object {
        ...
        fun <T : Any, P : Any> fromDecimal(displayQuantityDelta: BigDecimal,
            ...
            val tokenSize = Balance.getDisplayTokenSize(token)
     ...
     */
    fun toDecimal(): BigDecimal = BigDecimal.valueOf(quantityDelta, 0) * Balance.getDisplayTokenSize(token)
